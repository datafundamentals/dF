# DF Chat Temporary Test App

Offline-friendly host application cloned from the Firebase teaching app as a sandbox for experimenting with new chat features while keeping all Firebase Emulator Suite tooling intact.
...
3. Start the emulators against the shared demo project. This command keeps the suite running and persists data across restarts:
   ```sh
   pnpm --filter @df/df-chat-tmp-test-app emulators:start
   ```
4. Launch the web app in a second terminal:
   ```sh
   pnpm --filter @df/df-chat-tmp-test-app dev
   ```
5. Open `http://127.0.0.1:4177` in your browser. The banner in the landing page confirms whether the Emulator UI is reachable (port `4000`).
...
- Export current emulator state:
  ```sh
  pnpm --filter @df/df-chat-tmp-test-app emulators:export
  ```
  
- Import a saved snapshot and start emulators:
  ```sh
  pnpm --filter @df/df-chat-tmp-test-app emulators:import
  ```
...
1. Start the emulators: `pnpm --filter @df/df-chat-tmp-test-app emulators:start`
2. Launch the dev server: `pnpm --filter @df/df-chat-tmp-test-app dev`
...
- `pnpm --filter @df/df-chat-tmp-test-app build` – Type-check and emit static assets.
- `pnpm --filter @df/df-chat-tmp-test-app preview` – Serve the production build on port `4177`.
- `pnpm --filter @df/df-chat-tmp-test-app test` – Run the Playwright smoke test (ensures the shell renders).
- `pnpm --filter @df/df-chat-tmp-test-app test:rules` – Run automated security rules tests.
...
pnpm --filter @df/df-chat-tmp-test-app test:rules
```
...
```bash
pnpm --filter @df/df-chat-tmp-test-app deploy:rules
```
...
pnpm --filter @df/df-chat-tmp-test-app preview:prod
```
...
pnpm --filter @df/df-chat-tmp-test-app deploy:prod

# Or deploy components individually:
pnpm --filter @df/df-chat-tmp-test-app deploy:rules      # Security rules only
pnpm --filter @df/df-chat-tmp-test-app deploy:functions  # Cloud Functions only
pnpm --filter @df/df-chat-tmp-test-app deploy:hosting    # Hosting only
```
  ...
  deploy:
    ...
    steps:
      ...
      - name: Create .env.production from secrets
        working-directory: apps/df-chat-tmp-test-app
        run: |
      ...
      - name: Run security rules tests
        run: pnpm --filter @df/df-chat-tmp-test-app test:rules
      
      - name: Build production app
        run: pnpm --filter @df/df-chat-tmp-test-app build:prod
...
# builds bundle
pnpm --filter @df/df-chat-tmp-test-app build:bundle
...
cp -r apps/df-chat-tmp-test-app/dist/* path/to/11ty-site/public/firebase-app/
```
...
```toml
[build]
  command = "pnpm install && pnpm --filter @df/df-chat-tmp-test-app build:bundle"
  publish = "apps/df-chat-tmp-test-app/dist"
...
```json
{
  "buildCommand": "pnpm install && pnpm --filter @df/df-chat-tmp-test-app build:bundle",
  "outputDirectory": "apps/df-chat-tmp-test-app/dist",
  "framework": null
}
```
...
2. Build command: `pnpm install && pnpm --filter @df/df-chat-tmp-test-app build:bundle`
3. Build output directory: `apps/df-chat-tmp-test-app/dist`
