...
#### What Belongs in `apps/df-chat-tmp-test-app/`
...
```typescript
// ✅ CORRECT - apps/df-chat-tmp-test-app/src/config/firebase.config.ts
...
│     └─ YES → Add to apps/df-chat-tmp-test-app/src/components/
│
...
└─ Is it SEED DATA or TEACHING EXAMPLES?
   └─ YES → packages/firebase-emulator/emulator-data/
            or apps/df-chat-tmp-test-app/scripts/
```
...
**WRONG:**
```typescript
// apps/df-chat-tmp-test-app/src/config/firebase.config.ts
...
**WRONG:**
```typescript
// apps/df-chat-tmp-test-app/src/utils/firebase-init.ts
...
// apps/df-chat-tmp-test-app/src/main.ts
...
5. **Wire it up in teaching app** → Demo the patterns in `apps/df-chat-tmp-test-app/`
...
- `apps/df-chat-tmp-test-app/tests/integration/` - End-to-end flows with emulators
...
3. **Check the Firebase Teaching App** (`apps/df-chat-tmp-test-app/`) for examples
