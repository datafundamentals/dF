...
export class DfFirebaseTeachingApp extends LitElement {
    ...
    return html`
      ...
      <section aria-labelledby="emulator-setup">
        ...
        <ul>
          ...
          <li><code>pnpm --filter @df/df-chat-tmp-test-app emulators:start</code></li>
          <li><code>pnpm --filter @df/df-chat-tmp-test-app dev</code></li>
  ...
  private getChecklist(): ChecklistItem[] {
    return [
      ...
      {
        label: 'Run `pnpm --filter @df/df-chat-tmp-test-app emulators:start`',
        completed: this.emulatorStatus === 'connected',
      },
      {
        label: 'Run `pnpm --filter @df/df-chat-tmp-test-app dev`',
        completed: false,
      },
    ];
  }
