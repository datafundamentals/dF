 ...
 *
 * NOTE: This function actually lives in services/functions/ (not here).
 * This file is legacy - see guides/FUNCTIONS_PLACEMENT.md for current architecture.
...
function validateTitle(title) {
    ...
    const startsWithActionVerb = actionVerbs.some(verb => trimmed.toLowerCase().startsWith(verb));
...
export const createTodoAdvanced = functions.https.onCall({
    ...
    if (!request.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to create todos');
    }
    const userId = request.auth.uid;
    functions.logger.info('Creating advanced todo', { userId, title: request.data.title });
    // 2. Extract and validate data
    const { title, description = '', priority = 'medium', tags = [], dueDate = null, } = request.data;
    validateTitle(title);
    // Validate priority
    const validPriorities = ['low', 'medium', 'high'];
    if (!validPriorities.includes(priority)) {
        throw new functions.https.HttpsError('invalid-argument', `Priority must be one of: ${validPriorities.join(', ')}`);
    }
    ...
    if (dueDate) {
        try {
            ...
            throw new functions.https.HttpsError('invalid-argument', 'Due date must be a valid ISO date string');
        }
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to create todo. Please try again.');
    }
});
