...
3. Start the emulators against the shared demo project. This command keeps the suite running and persists data across restarts:
   ```sh
   pnpm --filter @df/df-firebase-teaching-app2 emulators:start
   ```
4. Launch the web app in a second terminal:
   ```sh
   pnpm --filter @df/df-firebase-teaching-app2 dev
   ```
...
- Export current emulator state:
  ```sh
  pnpm --filter @df/df-firebase-teaching-app2 emulators:export
  ```
  
- Import a saved snapshot and start emulators:
  ```sh
  pnpm --filter @df/df-firebase-teaching-app2 emulators:import
  ```
...
1. Start the emulators: `pnpm --filter @df/df-firebase-teaching-app2 emulators:start`
2. Launch the dev server: `pnpm --filter @df/df-firebase-teaching-app2 dev`
...
- `pnpm --filter @df/df-firebase-teaching-app2 build` – Type-check and emit static assets.
- `pnpm --filter @df/df-firebase-teaching-app2 preview` – Serve the production build on port `4176`.
- `pnpm --filter @df/df-firebase-teaching-app2 test` – Run the Playwright smoke test (ensures the shell renders).
- `pnpm --filter @df/df-firebase-teaching-app2 test:rules` – Run automated security rules tests.
...
pnpm --filter @df/df-firebase-teaching-app2 test:rules
```
...
```bash
pnpm --filter @df/df-firebase-teaching-app2 deploy:rules
```
...
pnpm --filter @df/df-firebase-teaching-app2 build:prod

# Preview production build locally
pnpm --filter @df/df-firebase-teaching-app2 preview:prod
```
...
pnpm --filter @df/df-firebase-teaching-app2 deploy:prod

# Or deploy components individually:
pnpm --filter @df/df-firebase-teaching-app2 deploy:rules      # Security rules only
pnpm --filter @df/df-firebase-teaching-app2 deploy:functions  # Cloud Functions only
pnpm --filter @df/df-firebase-teaching-app2 deploy:hosting    # Hosting only
```
  ...
  deploy:
    ...
    steps:
      ...
      - name: Create .env.production from secrets
        working-directory: apps/df-firebase-teaching-app2
        run: |
      ...
      - name: Run security rules tests
        run: pnpm --filter @df/df-firebase-teaching-app2 test:rules
      
      - name: Build production app
        run: pnpm --filter @df/df-firebase-teaching-app2 build:prod
...
# builds bundle
pnpm --filter @df/df-firebase-teaching-app2 build:bundle
...
cp -r apps/df-firebase-teaching-app2/dist/* path/to/11ty-site/public/firebase-app/
```
...
```toml
[build]
  command = "pnpm install && pnpm --filter @df/df-firebase-teaching-app2 build:bundle"
  publish = "apps/df-firebase-teaching-app2/dist"
...
```json
{
  "buildCommand": "pnpm install && pnpm --filter @df/df-firebase-teaching-app2 build:bundle",
  "outputDirectory": "apps/df-firebase-teaching-app2/dist",
  "framework": null
}
```
...
2. Build command: `pnpm install && pnpm --filter @df/df-firebase-teaching-app2 build:bundle`
3. Build output directory: `apps/df-firebase-teaching-app2/dist`
