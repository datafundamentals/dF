...
- **Canonical Types (single source of truth)** - A shared or persisted data shape is defined **exactly once** in `@df/types` and imported everywhere else. Never hand-copy the same field list into a store, component, test, or function — competing copies drift silently and force `as unknown as` casts. When a layer needs extra or narrowed fields, **extend** the canonical type (`interface Local extends Canonical { …delta… }`); do not re-declare it. See `guides/CANONICAL_TYPE_DEFINITIONS.md`.
...
- **No Authentication Emulator** - Auth Emulator remains forbidden for general development—**except one use case**: `apps/df-firebase-teaching-app` for email/password user creation testing. See app-specific docs for details. Rationale: "Authentication with email/password is burdensome for users"
