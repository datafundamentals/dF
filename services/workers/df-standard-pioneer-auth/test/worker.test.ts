...
describe('df-standard-pioneer-auth Worker', () => {
  it('returns the authenticated Access identity', async () => {
    ...
    const response = await handleRequest(
      request('/cf-auth/_protected/whoami'),
      context
    );
  ...
  it('returns the popup completion page', async () => {
    const response = await handleRequest(
      request('/cf-auth/_protected/login?mode=popup'),
  ...
  it('redirects only to the same origin', async () => {
    const safe = await handleRequest(
      request(
        '/cf-auth/_protected/login?returnTo=https%3A%2F%2Fexample.com%2Fdashboard'
      ),
      authenticatedContext
    );
    const unsafe = await handleRequest(
      request(
        '/cf-auth/_protected/login?returnTo=https%3A%2F%2Fevil.example%2F'
      ),
      authenticatedContext
    );
  ...
  it('rejects unsupported methods', async () => {
    const response = await handleRequest(
      request('/cf-auth/_protected/whoami', {method: 'POST'}),
      {}
    );
